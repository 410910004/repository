<template>
  <div>
    <el-input
        ...
        clearable
      />
    <el-button type="primary" icon="el-icon-plus" @click="openClass">
      添加
    </el-button>
    ...
    <el-table border :data="list" style="width: 100%; margin-top: 20px">
      <el-table-column
        ...
        align="center"
      ></el-table-column>
      <el-table-column prop="cname" label="班级名称" width="width"></el-table-column>
    ...
    <el-dialog title="添加班级" :visible.sync="dialogFormVisible">
      <!--
        表单验证
          ...
          prop:  表单验证的数据字段
      -->
      <el-form :model="className" :rules="rules" ref="className">
        <el-form-item label="班级名称" label-width="100px" prop="cname">
          <el-input v-model="className.cname" autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item label="人数" label-width="100px" prop="">
          <el-input v-model="className.num" autocomplete="off"></el-input>
        </el-form-item>
      </el-form>
...
export default {
  ...
  methods: {
    ...
    updateList(row){
      this.dialogFormVisible = true;
      this.className.id= row.id,
      this.className.cname= row.cname,
      this.className.num=row.num
    },
    // 修改数据或增加数据 
    updateOrAddClass(){
      if( this.className.id == 0 ){
          this.$axios.post("http://localhost:8083/class/save",this.className).then(res=>{
            const json = res.data;
            if (json.code==200){
              this.clearList();
              //刷新表格
          ...
          this.$axios.post("http://localhost:8083/class/mod",this.className).then(res=>{
              const json = res.data;
              if (json.code==200){
                this.clearList();
                //刷新表格
