<template>
  <div class="order-right">
    <div class="order-content">
      ...
      <div class="orders">
        <table class="order-item" v-for="order in orders.records" :key="order.id">
          <thead>
            <tr>
              <th colspan="5">
          ...
          <tbody>
            <tr v-for="(orders,index) in order.orderDetailList" :key="orders.id">
              <td width="60%">
...
export default {
  ...
  methods:{
    async getData(){
      let result = await this.$API.reqOrdersList(this.page,this.limit);
      if(result.code == 200){
